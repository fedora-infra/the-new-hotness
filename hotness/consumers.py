    ...
    def _handle_anitya_update(self, upstream, package, msg):
        ...
        if diff == 1:
            ...
            try:
                ...
                print (result_rh, rh_stuff)
                if int(result_rh) == 0:
                    if 'build_ref' in rh_stuff['build_logs']:
                        for version in versions:
                            for task_id in six.iteritems(rh_stuff['build_logs']['build_ref'][version]):
                                note = 'Scratch build for %s release started properly. %s' % (version,
                                                                                              '\n'.join(task_id))
                                self.log.info(note)
                                if 'old' == version:
                                    self.old_triggered_task_ids[task_id] = [bz, None, version, package]
                                else:
                                    self.new_triggered_task_ids[task_id] = [bz, None, upstream, package]
                else:
                    note = 'Patching or scratch build for %s and version %s FAILED. See for details' % (package, version)
